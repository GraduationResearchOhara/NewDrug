...
public class PatientActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                        ...
                        // 各ユーザーのデータを取得
                        String fullName = snapshot.child("fullName").getValue(String.class);
                        Integer birthYear = snapshot.child("birthYear").getValue(Integer.class);  // 修正箇所
                        Integer birthMonth = snapshot.child("birthMonth").getValue(Integer.class);  // 修正箇所
                        Integer birthDay = snapshot.child("birthDay").getValue(Integer.class);  // 修正箇所
                        String hospitalName = snapshot.child("hospitalName").getValue(String.class);
                        String email = snapshot.child("email").getValue(String.class);
                        String userId = snapshot.getKey();
                        // データがnullでないことを確認してから処理を続ける
                        if (birthYear != null && birthMonth != null && birthDay != null) {
                            ...
                            userTextView.setOnClickListener(v -> {
                                // クリックされたユーザーのIDを取得
                                String selectedUserId = snapshot.getKey();
                            ...
                            // データがnullの場合の処理
                            Toast.makeText(com.example.newdrug.PatientActivity.this, "生年月日データが不足しています", Toast.LENGTH_SHORT).show();
                        }
                    }
                } else {
                    // データが存在しない場合の処理
        ...
        bottomNavigationView.setOnItemSelectedListener(item -> {
            ...
            if (itemId == R.id.navigation_home) {
                // ホームアクティビティの処理
                startActivity(new Intent(PatientActivity.this, Home.class));
                return true;
            } else if (itemId == R.id.navigation_book) {
                // MedicineListActivityに移動
                startActivity(new Intent(PatientActivity.this, SelectMode.class));
                return true;
            } else if (itemId == R.id.navigation_person) {
                // PersonalInformationActivityに移動
                startActivity(new Intent(PatientActivity.this, AccountManagementActivity.class));
